...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <password> [cost]" << std::endl;
        std::cerr << "  cost: bcrypt cost factor (4-31, default 12)" << std::endl;
        std::cerr << "  output: bcrypt hash to stdout" << std::endl;
        return 1;
    }
    ...
    if (argc >= 3) {
        cost = static_cast<unsigned int>(std::stoi(argv[2]));
    }
    if (cost < 4) cost = 4;
    if (cost > 31) cost = 31;
    ...
    std::string hash = bcrypt_hashpw(password, salt, cost);
