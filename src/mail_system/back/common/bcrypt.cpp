...
static std::array<unsigned char, 128> build_b64_decode() {
    std::array<unsigned char, 128> t{};
    for (auto& c : t) c = 0xFF;
    for (unsigned char i = 0; i < 64; ++i)
        t[static_cast<size_t>(kBase64Code[i])] = i;
    return t;
}
static const auto kB64Dec = build_b64_decode();

// Encode 'len' bytes (must decode to exactly 'out_chars' chars)
...
//   且 verify 对任意密码都返回 true（round-trip 自洽掩盖）。2026-08-29 修复。
...
// Decode 'len' b64 chars → raw bytes
static void b64_decode(unsigned char* dst, const char* src, int len) {
    unsigned int val = 0;
    int bits = 0;
    for (int i = 0; i < len; ++i) {
        unsigned char c = static_cast<unsigned char>(src[i]);
        val = (val << 6) | (c < 128 ? kB64Dec[c] : 0);
        bits += 6;
        if (bits >= 8) {
            bits -= 8;
            *dst++ = static_cast<unsigned char>((val >> bits) & 0xFF);
        }
    }
}
...
// Blowfish cipher
// =========================================================================
using BFWord = unsigned int;

static const BFWord kInitP[18] = {
...
static const BFWord kInitS[4][256] = {
...
static inline BFWord bf_f(const BlowfishCtx& ctx, BFWord x) {
    return ((ctx.S[0][x >> 24] + ctx.S[1][(x >> 16) & 0xFF])
...
// =========================================================================
// Core: hash password with given salt
// =========================================================================
static std::string bcrypt_hashpw(const std::string& password,
                                 const unsigned char salt[16],
                                 unsigned int cost) {
    // Expand password (truncate at 72, null-pad)
    unsigned char key[72] = {};
    int key_len = static_cast<int>(password.size());
    if (key_len > 72) key_len = 72;
    std::memcpy(key, password.data(), key_len);
    if (key_len < 72) {
        key[key_len] = 0;
        ++key_len;
    }
    ...
    BlowfishCtx ctx;
    eks_blowfish_setup(ctx, salt, key, key_len, cost);

    // Magic string: "OrpheanBeholderScryDoubt" (24 bytes)
    ...
    // ⚠ 旧实现只加密 magic 前 8 字节(1 个块) → 与标准 bcrypt 不兼容，且产物 hash 段后半
    //   全是零填充点。2026-08-29 修复（对齐 OpenBSD/jBCrypt 行为）。
    BFWord cdata[6];
    for (int i = 0; i < 6; ++i) {
    ...
    char cost_str[4];
    std::snprintf(cost_str, sizeof(cost_str), "%02u", cost);
    ...
    result += cost_str;
    result += "$";
    result.append(b64_salt, 22);
    result.append(b64_hash, 31);
    return result;
}
...
std::string bcrypt_hash(const std::string& password, unsigned int cost) {
    if (cost < 4) cost = 4;
    if (cost > 31) cost = 31;
    ...
    return bcrypt_hashpw(password, salt, cost);
}

bool bcrypt_verify(const std::string& password, const std::string& hash) {
    // Must be $2b$ or $2a$ or $2x$ or $2y$ format
    if (hash.size() < 60) return false;
    if (hash[0] != '$' || hash[1] != '2' || hash[3] != '$') return false;
    if (hash[2] != 'a' && hash[2] != 'b' && hash[2] != 'x' && hash[2] != 'y')
        return false;

    // Parse cost (2 digits)
    unsigned int cost = static_cast<unsigned int>(
        (hash[4] - '0') * 10 + (hash[5] - '0'));
    if (cost < 4 || cost > 31) return false;
    if (hash[6] != '$') return false;

    // Parse salt (22 chars at position 7)
    const char* salt_enc = hash.c_str() + 7;
    unsigned char salt[16];
    b64_decode(salt, salt_enc, 22);

    // Re-hash and compare
    std::string computed = bcrypt_hashpw(password, salt, cost);
    if (computed.empty() || computed.size() < 60) return false;

    // Constant-time comparison on the hash portion (position 29 onward, 31 chars)
    // Simple memcmp is OK here since we're comparing self-computed values
    return std::memcmp(computed.c_str() + 29, hash.c_str() + 29, 31) == 0;
}
